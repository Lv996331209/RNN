...
def plot_results(history, model, Y_target, coin):
    ...
    real =pd.DataFrame(test_set[coin + '_Close**'][window_len:].values.tolist())
    real.to_csv('real.csv')
    pred=pd.DataFrame(((np.transpose(model.predict(X_test)) + 1) * test_set[coin + '_Close**'].values[:-window_len])[0])
...
#
# X_train=pd.DataFrame(btc_model.predict(X_train))
# X_train.to_csv('X_train.csv')
# X_test=pd.DataFrame(btc_model.predict(X_test))
